...
const ChatMessage: React.FC<Props> = ({ message, author, serverId }) => {
  ...
  return (
    ...
    >
      <div className="flex">
        <span className="text-gray-light text-xs w-14">{formattedTime}</span>
        <span className="text-lg px-2 font-bold">{author.username}</span>
      </div>
      <div className="flex">
        <span className="font-light text-bottom mr-2 break-all">
          {message.content}
        </span>
      </div>
